...
在 aiMindServe 中，`llama-box` 和 `vox-box` 后端支持 NVIDIA CUDA 推理。

## 支持的设备

- [x] NVIDIA GPU（需支持 CUDA）
...
docker run -d --name aimindserve \
    ...
    aimindserve/aimindserve:latest-nvidia-cuda
```
...
你可以为 aiMindServe 添加更多 NVIDIA GPU 节点。需在其他 NVIDIA GPU 节点上添加工作节点，并指定 `--server-url` 和 `--token` 参数加入 aiMindServe
